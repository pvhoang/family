...
export class TreePage implements OnInit {
  ...
  //       const options = { bgcolor: 'white', width: ele.clientWidth, height: ele.clientHeight, quality: 1.0 };
	...
	// 			domtoimage.toSvg(ele, {filter: this.filter}).then((dataUrl:any) => {
	// 				if (DEBUGS.TREE)
	// 					console.log('onPDF - dataUrl: ', dataUrl);
  //         this.getPDFImages(dataUrl, ele).then((dim: any) => {
	// 					if (DEBUGS.TREE)
  ...
  // }
    ...
    this.utilService.alertConfirm('ANNOUNCE', msg, 'CANCEL', 'CONTINUE').then((res) => {
      if (res.data) {
        ...
        const options = { bgcolor: 'white', width: ele.clientWidth, height: ele.clientHeight, filter: this.filter, quality: 1.0 };

				// if (DEBUGS.TREE)
				// 	console.log('onPDF: ele: ', ele);
				// domtoimage.toBlob(ele, options).then((blob:any) => {
				// 		// saveAs(blob, 'my-node.png');
				// 	console.log('onPDF: blob: ', blob);
				// })
				// .catch((error:any) => {
				// 	console.log('onPDF - blog error: ', error);
				// })

        domtoimage.toPng(ele, options).then((dataUrl:any) => {
					// if (DEBUGS.TREE)
					// 	console.log('onPDF - imgData: ', imgData);
					// const img = new Image();
					// img.src = dataUrl;
  ...
  onPDF_TEST() {
	...
	private getPDFImages_TEST(imgData: any, ele: any) {
    return new Promise((resolve) => {
      const img = new Image();
      img.src = imgData;
      // load img to canvas
      img.onload = () => {
				let data = { x: 10, y: 10,  w: ele.clientWidth, h: ele.clientHeight, canvas: '' }
	...
	private getPDFDimension_TEST(ele: any) {
		// first page
		let images = [];
    images.push({ 
      page: { x: 5, y: 10, w: ele.clientWidth, h: ele.clientHeight },
      image: { x: 0, y: 0, w: ele.clientWidth, h: ele.clientHeight }
    })
    return { images: images };
  }

	private printPDF_TEST(data: any, fileName: any, node: any) {
    ...
    // 1st page
    doc.addImage(url, "PNG", 0, 0, data.w, data.h);
    this.printText(node, doc, data.w, data.h, 5 + 10);
    doc.addImage(data.canvas, 'PNG', data.x, data.y, data.w, data.h);
  ...
  private getPDFDimension(ele: any) {
    // use A4: width = 210mm, height = 297mm, landscape
    // let pageWidth = 210;
    // let pageHeight = 297;
		// let marginLeft = 30;
    // let marginRight = 30;
    ...
    let textHeight = 20;
			...
			// convert to mm
			let clientWidth = ele.clientWidth * 0.2645833333;
			let clientHeight = ele.clientHeight * 0.2645833333;

    let xImage = 0;
    let wImage = clientWidth; // ele.clientWidth
    // calculate ratio between image and page
    let imageWidth = pageWidth - (marginLeft + marginRight);
    let wRatio = imageWidth / clientWidth;
    let imageHeight = parseInt(''+wRatio * clientHeight);
    let hRatio = clientHeight / imageHeight;
