...
*/
	...
	// Ten Quan height
		...
		private getRelationHtml() {
			...
			if (children.length > 0) {
				...
				html += '<ion-col size="4" class="column center"><span class="label">Tên</span></ion-col>';
				html += '<ion-col size="2" class="column center"><span class="label">Quan hệ</span></ion-col>';
				html += '<ion-col size="6" class="column center"><span class="label">Thông tin</span></ion-col>';
				html += '</ion-row>';
				...
				console.log('item: ', item);
					...
					if (note == '') {
						let row = '<ion-row>';
						// let row = rowStart;
						row += '<ion-col size="4" class="column center" ' + color + '><b>' + name + '</b></ion-col>';
						row += '<ion-col size="2" class="column center"><b>' + relation + '</b></ion-col>';
						row += '<ion-col size="6" class="column"><b></b></ion-col>';
						row += '</ion-row>';
				...
				console.log('row: ', row);
						...
						notes.forEach((no:any) => {
							...
							let row = '<ion-row>';
							row += '<ion-col size="4" class="column center" ' + color + '><b>' + n + '</b></ion-col>';

							// row += '<ion-col size="4" class="column center"><b>' + n + '</b></ion-col>';
							row += '<ion-col size="2" class="column center"><b>' + rel + '</b></ion-col>';
							row += '<ion-col size="6" class="column left"><b>' + no + '</b></ion-col>';
							row += '</ion-row>';
							html += row;
							first = false;
						})
	...
	getImageHtml(images: any, str: any, textarea?: boolean) {
		...
		// console.log('sizeValues[size]: ', sizeValues[size]);

		let data = { width: sizeValues[size].width, height: sizeValues[size].height };
		// console.log('data: ', data);
		// console.log('data: ', images[name]);
			// if defined
