...
export class EditorService {
  ...
  constructor(private utilService: UtilService
	...
	convertArrayToHtml(images: any, lines: any, textarea?: boolean) {
		...
		for (let il = 0; il < lines.length; il++) {
			...
			if (line.indexOf('<im>') == 0 && line.indexOf('</im>') > 0)
				...
				// decode document formatting
				paraHtml = this.getUIHtml(line, images, ['DOCUMENT', '<do>', '</do>']);
			else if (line.indexOf('<vi>') == 0 && line.indexOf('</vi>') > 0)
				// decode video formatting
				paraHtml = this.getUIHtml(line, images, ['VIDEO', '<vi>', '</vi>']);
			// else if (line.indexOf('<te>') == 0 && line.indexOf('</te>') > 0)

			else if (line.indexOf('<te>') == 0) {
				// paraHtml = this.getUIHtml(line, images, ['TEXT', '<te>', '</te>']);
				html = this.getTextHtml(line)
			}
					// decode text formatting
				
			else
				// regular text, decode line formatting
	...
	private getTextHtml(str: any) {

		// // .viewer-home-container-left{
		// // 	display: flex;
		// // 	justify-content: var(--app-viewer-home-justify-content);
		// // 	color: var(--app-viewer-home-color);
		// // 	font-weight: var(--app-viewer-home-font-weight);
		// // 	font-size: var(--app-viewer-home-font-size);
		// // }

		// align (left, right, center), padding (left 10, 20, 30px), strong, italic, underline, small, normal, large
		// green, red, blue
		// const specialChars = ['al', 'ar', 'ac', '10', '20', '30', 's','e','u','3','2','1','g','r', 'b']
		// <te>1|Hello
		let items = str.split('|');
		let html = '<div class="viewer-home-container-type-' + items[0].substring(4) + '">' + items[1] + '</div>';
		console.log('html: ', html);
		return html;
	}
