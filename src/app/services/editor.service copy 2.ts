  ...
  constructor(private utilService: UtilService
	...
	convertArrayToHtml(images: any, lines: any, textarea?: boolean) {
		...
		for (let il = 0; il < lines.length; il++) {
			...
			// 	if (popup) {
			// 		// end of popup
			// 		popupHtmlEnd = html.length;
			// 		popupHtml = popupHtml + '|' + html.substring(popupHtmlStart, popupHtmlEnd);

			// 	}
			// 	popup = true;
			// 	popupHtml = line;
			// 	popupHtmlStart = html.length;

			// 	// this.setPopupHtml(line);
			// 	continue;
			} else {
	...
	// private setPopupHtml(line) {
	// 	this.popupData.push(line);
	// }
