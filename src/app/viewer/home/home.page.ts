  ...
  ngOnInit() {
	...
	start() {
		this.dataService.readAncestorData().then((dat:any) => {
			if (DEBUGS.HOME)
				console.log('HomePage-start: dat: ', dat);
	...
	updatePageData(docs: any) {
		...
		for (var key of Object.keys(docs)) {
			...
			html = this.replaceViewTemplate(html);
			let htmls = this.replacePopupTemplate(html);

			if (DEBUGS.HOME)
				console.log('HomePage-updatePageData: key, htmls: ', key, htmls);
	...
	getTableTrees() {
		...
		this.nodes.forEach((node: any) => {
			let tree = { id: node.id, name: node.name, all: '', gen: node.level, branch: '', sub_branch: '', sub_sub_branch: '' };
			if (node.root) {
				...
				tree = null;
			if (tree) {
				tree.all = tree.gen + '/' + tree.branch + '/' + tree.sub_branch + '/' + tree.sub_sub_branch;
				trees.push(tree);
			}
		});
		return trees;
	...
	private replacePopupTemplate(html: any) {
		let htmls = [];
		// console.log('html: ', html);
		let dataOK = true;
		let i1 = 0;
