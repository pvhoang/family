...
export class SearchPage implements OnInit {
  ...
  constructor(
	...
	start() {
		...
		this.items = [];
    this.itemPlaceholder = this.languageService.getTranslation('SEARCH_PLACEHOLDER');
    ...
    this.nodes.forEach((node: any) => {
			...
			if (Array.isArray(node.desc)) {
				...
				// does not make sense! Because search start from first word, eg : 'Ông Phan Gia Trưng và Bà Hoàng Thị Tha sinh hạ ...'
			...
			if (name == 'Phan Tất Đắc') {
				console.log('snode: ', snode);
			}
      searchNodes.push(snode);
    })

		let sFields = [ 'name', 'nick', 'pob', 'pod', 'por', 'yob', 'yod', 'dod' ];
		// add desc fields for name
		searchDescFields.forEach(field => {
			sFields.push(field);
		})
		// remove duplicate
		let searchFields = sFields.filter(function(item, pos) {
			return sFields.indexOf(item) == pos;
		})
  ...
  keyupItem(event) {
	...
	private getNodeHtml(node: any, match: any) {
		...
		if (node.nick != '')
			html +=
			'<ion-row>' +
				'<ion-col size="5" class="column center">' + this.languageService.getTranslation('NODE_NICK') +
				'</ion-col>' +
				'<ion-col size="7" class="column center">' + this.getMatch(node.nick, match) +
				'</ion-col>' +
			'</ion-row>';
