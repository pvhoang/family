...
const VERTICAL_TREE = 0;
const HORIZONTAL_TREE = 1;
...
export class VnodePage implements OnInit {
  ...
  treeIcon = FONTS_FOLDER + 'family-tree.svg'

	// treeClass = 'horizontal-tree'
  // viewMode = HORIZONTAL_TREE;
  ...
  onViewChange() {
		if (this.viewMode == VERTICAL_TREE) {
			this.treeClass = 'horizontal-tree';
			this.viewMode = HORIZONTAL_TREE;
 			this.treeIcon = FONTS_FOLDER + 'arrow-split-vertical.svg'
		} else {
			this.treeClass = 'vertical-tree';
			this.viewMode = VERTICAL_TREE;
			this.treeIcon = FONTS_FOLDER + 'family-tree.svg'
		}
	}
